...
export function detectMediaType(bytes) {
  ...
  if (bytesEqual(bytes, [0x66, 0x74, 0x79, 0x70], 4)) {
    ...
    if (/avif|avis/.test(brands)) return { kind: "image", contentType: "image/avif", extension: "avif" };
    if (/heic|heix|hevc|hevx/.test(brands)) return { kind: "image", contentType: "image/heic", extension: "heic" };
    if (/mif1|msf1|heim|heis|hevm|hevs/.test(brands)) return { kind: "image", contentType: "image/heif", extension: "heif" };
    if (/isom|iso2|mp4[12]|avc1|M4V |MSNV|dash/.test(brands)) return { kind: "video", contentType: "video/mp4", extension: "mp4" };
  }
...
export function validMediaKey(key) {
  return /^(?:image|audio|video)\/\d{4}\/\d{2}\/\d{2}-[0-9a-f-]+-(?:single|original|optimized)\.(?:jpe?g|png|gif|webp|avif|heic|heif|mp3|mp4)$/i.test(String(key || ""));
}
