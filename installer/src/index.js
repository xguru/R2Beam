...
async function configuration(env, request) {
  ...
  return page(`<h1>R2Beam을<br>설치하세요.</h1><p>미디어는 선택한 Cloudflare 계정의 R2에 저장됩니다.</p><form class="card" method="post" action="/install"><label>Cloudflare 계정<select name="accountId">${options}</select></label><label>Worker 이름<input name="workerName" value="r2beam-${suffix}" required></label><label>R2 버킷 이름<input name="bucketName" value="r2beam-media-${suffix}" required></label><label>커스텀 도메인 <small>선택 사항</small><input name="customHostname" list="zone-suggestions" placeholder="vault.example.com"><small>비워두면 workers.dev 주소를 사용합니다.${zoneNames ? ` 사용 가능한 Zone: ${zoneNames}` : " 현재 계정에서 활성 Zone을 찾지 못했습니다."}</small></label><datalist id="zone-suggestions">${zoneOptions}</datalist><p class="meta">관리자: ${escapeHtml(session.email)}</p><button>이 계정에 설치</button></form>`);
}
...
async function install(env, request) {
  ...
  const result = await installR2Beam({ accessToken: session.accessToken, accountId, ownerEmail: session.email, workerName, bucketName, customDomain, releases: env.RELEASES });
