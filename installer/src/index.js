import { CloudflareError, client, installR2Beam } from "./cloudflare.js";
...
function r2ActivationRequiredPage({ accountId, workerName, bucketName, customHostname }) {
  const hidden = [
    ["accountId", accountId],
...
async function configuration(env, request) {
  ...
  const suffix = session.accounts[0].id.slice(0, 6);
  const options = session.accounts.map((account) => `<option value="${escapeHtml(account.id)}">${escapeHtml(account.name)}</option>`).join("");
  const zoneOptions = (session.zones || []).map((zone) => `<option value="r2beam.${escapeHtml(zone.name)}"></option>`).join("");
  const zoneNames = (session.zones || []).map((zone) => escapeHtml(zone.name)).join(", ");
  return page(`<h1>R2Beam을<br>설치하세요.</h1><p>미디어는 선택한 Cloudflare 계정의 R2에 저장됩니다.</p><form class="card" method="post" action="/install"><label>Cloudflare 계정<select name="accountId">${options}</select></label><label>Worker 이름<input name="workerName" value="r2beam-${suffix}" required></label><label>R2 버킷 이름<input name="bucketName" value="r2beam-media-${suffix}" required></label><label>커스텀 도메인 <small>선택 사항</small><input name="customHostname" list="zone-suggestions" placeholder="vault.example.com"><small>비워두면 workers.dev 주소를 사용합니다.${zoneNames ? ` 사용 가능한 Zone: ${zoneNames}` : " 현재 계정에서 활성 Zone을 찾지 못했습니다."}</small></label><datalist id="zone-suggestions">${zoneOptions}</datalist><p class="r2-note"><strong>새 Cloudflare 계정인가요?</strong> R2는 무료 월간 사용량을 제공하지만 최초 한 번 구독 활성화가 필요합니다. <a class="text-link" href="${R2_OVERVIEW_URL}" target="_blank" rel="noopener noreferrer">R2 활성화하기 ↗</a></p><p class="meta">관리자: ${escapeHtml(session.email)}</p><button>이 계정에 설치</button></form>`);
}
...
async function install(env, request) {
  ...
  try {
    ...
    if (isR2NotEnabledError(error)) {
      return r2ActivationRequiredPage({ accountId, workerName, bucketName, customHostname: customDomain?.hostname || "" });
    }
    throw error;
  }
  ...
  return page(`<h1>설치가<br>완료되었습니다.</h1><p>R2Beam ${escapeHtml(result.version)}이 설치되었습니다.${readiness}${result.customDomain ? ` 커스텀 도메인 인증서가 준비되는 데 잠시 걸릴 수 있습니다.` : ""} 처음 접속하면 설치에 사용한 Cloudflare 계정으로 로그인합니다.</p><div class="card"><a class="button" href="${escapeHtml(result.url)}">내 미디어 볼트 열기</a></div>`);
}
