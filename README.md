...
- 생성된 `/media/*` 링크만 공개
- 원본만 / 최적화본만 / 둘 다 보관 선택
- 이미지: JPEG, PNG, GIF, WebP, AVIF, HEIC/HEIF 입력, WebP 품질 78 우선, 미지원 브라우저는 JPEG/PNG 폴백, 긴 변 960px, 확대·크롭 없음, 방향 반영 및 메타데이터 제거
- 영상: H.264 MP4, 긴 변 960px, 최대 30fps, CRF 29, AAC 96kbps, Fast Start
- 영상 재생을 위한 HTTP Range 요청 지원

HEIC/HEIF는 Safari 17 이상처럼 브라우저가 해당 형식을 디코딩할 수 있을 때 게시판용
WebP/JPEG로 변환됩니다. 디코딩 미지원 환경에서도 **원본만** 저장할 수 있습니다.
...
같은 Cloudflare 계정에서 설치기를 다시 실행하면 기존 R2Beam Worker와 `MEDIA` R2
바인딩을 찾아 업그레이드 화면을 표시합니다. 기존 Worker 이름, 버킷과 커스텀 도메인을
자동으로 재사용하므로 저장된 미디어와 공개 링크는 유지됩니다.

설치된 R2Beam은 관리 화면을 열 때 중앙 설치기의 공개 버전 정책만 확인합니다.
일반 업데이트는 배너로 안내하고, 최소 지원 버전보다 오래된 필수 업데이트는 업로드와
삭제 화면을 차단한 뒤 설치기 링크를 표시합니다. 어떤 경우에도 기존 `/media/*` 공개
링크는 계속 제공됩니다. 버전 확인에 실패하면 기존 기능을 그대로 허용하며 사용자나
Cloudflare 계정 정보는 중앙 설치기로 전송하지 않습니다.
